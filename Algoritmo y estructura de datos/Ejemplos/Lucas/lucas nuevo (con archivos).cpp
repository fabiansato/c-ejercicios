...
void FuncionArchivo(int *tiposobre,int *edad,int *peso,int *operacion,int *atendido,int *cant_sobres,char *sexo, int *vacuna);
...
void FuncionMenu(int *tiposobre,int *edad,int *peso,int *operacion,int *atendido,int *cant_sobres,char *sexo, int *vacuna)
{
	...
	case 4:
	FuncionArchivo(tiposobre,edad,peso,operacion,atendido,cant_sobres,sexo,vacuna);	
		}
	}while(elige!=5);
...
void FuncionArchivo(int *tiposobre,int *edad,int *peso,int *operacion,int *atendido,int *cant_sobres,char *sexo, int *vacuna){
	int m;
	FILE *flujo; 
	flujo = fopen ("listaveterinaria.txt","w+");
			if (flujo == NULL){
	...
	fprintf(flujo,"-----------------------------------------------------------------------------\n");
	fprintf(flujo,"|Dia\t|Atendido ant.\t|Sobre\t|Cant. sobres\t|Sexo\t|Edad\t|Peso\t|Vacuna\t|Operacion\n");
	fprintf(flujo,"-----------------------------------------------------------------------------\n");
		 
 	for(m=1;m<=DIAS;m++){
	fprintf(flujo,"|%i\t|%i v.\t\t|%i\t|%i\t\t|%c\t|%i\t|%i\t|%i\t|%i\n",m,atendido[m],tiposobre[m],cant_sobres[m],sexo[m],edad[m],peso[m],vacuna[m],operacion[m]);
